    ...
    >
      ...
      <div class="w-320 flex-col px-20 py-32">
        ...
        <div class="mt-20 flex items-center">
          <n-input
            v-model:value="loginInfo.answer"
...
async function getCaptcha() {
  const { b64s, id } = await api.getCaptcha()

  captchaUrl.value = b64s
  loginInfo.value.id = id
}
const initCaptcha = throttle(() => {
  getCaptcha()
  // captchaUrl.value = `${import.meta.env.VITE_AXIOS_BASE_URL}/api/public/index/login`
}, 500)
...
async function handleLogin() {
  const { username, password, answer, id } = loginInfo.value
  ...
  try {
    ...
    const res = await api.login({ username, password: password.toString(), answer, id })
    if (isRemember.value) {
